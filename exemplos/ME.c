...
main(int argc, char** argv)
{
    ...
    if ( my_rank == 0 ) // qual o meu papel: sou o mestre ou um dos escravos?
    {
     ...
     else
     {
         ...
         printf("[%d]receiving work: %d\n",my_rank,message);
         message = message+1;      // icremento conte̼do da mensagem
         printf("[%d]sending work: %d\n",my_rank,message);
         MPI_Send(&message,  8, MPI_INT,0, 0, MPI_COMM_WORLD);    // retorno resultado para o mestre
         printf("[%d]sended work: %d\n",my_rank,message);
     }
